...
function TrackCard({ variant, name, tag, promise, verticals, cta, onSmoothScroll, trackEvent, eventName }) {
  return (
    <article className={`track-card track-card--${variant} fade-in`}>
  ...
  return (
    <section className="section section--dark" id="two-track">
      <div className="container">
        <div className="section-label fade-in">{t.track_label}</div>
        <h2 className="section-title fade-in" dangerouslySetInnerHTML={{ __html: t.track_title }} />
        <p className="section-subtitle fade-in">{t.track_sub}</p>
